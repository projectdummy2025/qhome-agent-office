...
def _get_stock_and_details_for_sku(sku: str) -> dict:
    """Mengambil data stock_qty, base_price, coverage_m2, nama, dan deskripsi langsung dari database SQL."""
...
def _get_candidates_with_stock(query_text: str, category: str, limit: int = 5) -> list:
    """Mendapatkan kandidat produk dari ChromaDB beserta stok terupdate dari SQL."""
    from backend.core.database import get_chroma_collection
    try:
...
def _format_candidates_for_prompt(candidates: list) -> str:
    """Memformat daftar kandidat agar mudah dibaca oleh LLM dalam prompt."""
    lines = []
...
def tile_estimator(state: AgentState):
    ...
    try:
        ...
        if reuse_result["should_reuse"] and reuse_result["product"]:
            ...
            # Lakukan pencarian baru di ChromaDB dengan 5 kandidat
                ...
                if prod_name and prod_name != "Menunggu Konfirmasi":
                    ...
                    if db_product:
                        ...
                        if qty_val >= 20 and qty_val >= qty_int:
                            ...
                            # Bersihkan prefix stok lama agar tidak menumpuk
